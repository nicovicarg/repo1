...
function problemB()
{
  ...
  Promise.all([1, 2, 3, 4, 5, 6, 7, 8].map(n =>
    promisifiedReadFile(`poem-two/stanza-0${n}.txt`)
  ))
    .then((p2stanzas) =>
    {
      (p2stanzas).forEach((n) => blue(n))
    })
    .done(() => console.log('done'))
...
function problemC()
{
  ...
  // ???
  [2, 3, 4, 5, 6, 7, 8]
    .map(n => promisifiedReadFile(`poem-two/stanza-0${n}.txt`))
    .reduce((promesaAnterior, promesaActual) =>
    {
      promesaAnterior.then((stanza) =>
      {
        blue(stanza);
